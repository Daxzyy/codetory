...
import { Search, ArrowLeft, Copy, Download, ExternalLink, Check, FileCode, ChevronRight, Plus, Loader2, Eye, EyeOff } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center group">
          <svg width="130" height="34" viewBox="20 55 340 90" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="168" y="115" fill="#fff" fontSize="40" fontFamily="Ubuntu Mono, monospace" fontWeight="700" letterSpacing="1.5">odetory</text>
          </svg>
        </Link>
      </div>
    </nav>
...
function Home() {
  ...
  useEffect(() => {
    fetch("https://raw.githubusercontent.com/Daxzyy/codetory/main/public/data/scripts.json")
      ...
      .catch((err) => { console.error("Failed to load scripts", err); setLoading(false); });
  }, []);
...
function ViewScript() {
  ...
  useEffect(() => {
    ...
    const fetchMetadata = fetch("https://raw.githubusercontent.com/Daxzyy/codetory/main/public/data/scripts.json")
      .then(res => res.json())
      .then(data => { const script = data.find((s: any) => s.fileName === fileName); setScriptData(script); });
    const fetchCode = fetch(`https://raw.githubusercontent.com/Daxzyy/codetory/main/public/scripts/${fileName}`)
      .then((res) => { if (!res.ok) throw new Error("Script not found"); return res.text(); });
    Promise.all([fetchMetadata, fetchCode])
      .then(([_, text]) => { setCode(text); setLoading(false); })
          ...
          <div className="border border-white/10 rounded-lg overflow-hidden" style={{ background: '#161616' }}>
            <div className="flex items-center justify-between px-3 py-1.5 bg-white/[0.02] border-b border-white/5">
              ...
              <div className="flex items-center gap-0">
                <a href={"/raw/" + fileName} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1 text-white/30 hover:text-white transition-all text-[10px] font-bold tracking-tight">
      ...
      {!authed ? (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center pt-16">
          <div className="flex flex-col gap-4 w-full max-w-xs">
            ...
            <button onClick={handleAuth} disabled={authLoading} className="px-4 py-2 border border-white/20 bg-white/5 hover:bg-white/10 text-white text-sm font-bold transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input name="name" value={form.name} onChange={handleChange} placeholder="Catbox Uploader" className="bg-white/5 border border-white/10 px-3 py-2 text-sm text-white focus:outline-none focus:border-white/30 transition-all" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-mono text-neutral-400 uppercase tracking-wider">File Name</label>
              <input name="fileName" value={form.fileName} onChange={handleChange} placeholder="catbox.js" className="bg-white/5 border border-white/10 px-3 py-2 text-sm text-white focus:outline-none focus:border-white/30 transition-all" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-mono text-neutral-400 uppercase tracking-wider">Language</label>
              <div className="relative">
                <select name="language" value={form.language} onChange={handleChange} className="w-full appearance-none bg-white/5 border border-white/10 px-3 py-2 text-sm text-white focus:outline-none focus:border-white/30 transition-all pr-8">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea name="explanation" value={form.explanation} onChange={handleChange} placeholder="Script ini berfungsi untuk..." rows={3} className="bg-white/5 border border-white/10 px-3 py-2 text-sm text-white focus:outline-none focus:border-white/30 transition-all resize-none" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-mono text-neutral-400 uppercase tracking-wider">Code</label>
              <textarea name="code" value={form.code} onChange={handleChange} placeholder="const x = ..." rows={12} className="bg-white/5 border border-white/10 px-3 py-2 text-sm text-white font-mono focus:outline-none focus:border-white/30 transition-all resize-none" />
            </div>
            ...
            <button onClick={handleSubmit} disabled={status === "loading" || status === "success"} className="flex items-center justify-center gap-2 px-4 py-2.5 border border-white/20 bg-white/5 hover:bg-white/10 text-white text-sm font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed">
